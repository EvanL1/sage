    ...
    let presets: &[(&str, &str, &str, &str, &str, &str, &str, i32, &str, bool)] = &[
        ...
        (
            ...
            concat!(
                ...
                "注意：晋升/降级认知层级现在由 integrator 阶段负责，本阶段不处理层级变更。\n\n",
                "## 通用规则\n",
                "- 不确定时不操作\n",
                "- 输出 NONE 如果没有需要处理的内容",
            ),
        ...
        // ── Verifier / Contradiction / Integrator ─────────────────
        (
            "verifier",
            "认知验证：用今日证据检验已有模式判断",
            concat!(
                "你是 Sage 的认知验证器。你的工作是拿今日的观察证据，检验以下已有认知是否仍然成立。\n\n",
                "## 今日观察\n{context}\n\n",
                "对每条待验证认知，输出一个判断：\n",
                "- ACTION verify_confirm | memory_id | evidence_summary\n",
                "  （今日有证据支持这个认知）\n",
                "- ACTION verify_challenge | memory_id | counter_evidence\n",
                "  （今日有证据可能与这个认知矛盾——描述矛盾点）\n",
                "- 如果今日没有相关证据，跳过不输出\n\n",
                "规则：\n",
                "- 只基于今日实际观察判断，不要推测\n",
                "- CHALLENGE 不代表认知一定错，只代表出现了不一致的信号\n",
                "- 宁可跳过也不要勉强输出\n",
                "- 输出 NONE 如果没有相关证据",
            ),
            "", // insert_after（由 DAG edges 控制）
        ...
        (
            ...
            "矛盾检测：找出互相矛盾的认知",
            concat!(
                "你是 Sage 的矛盾检测器。审查以下认知记忆，找出互相矛盾的对。\n\n",
                "## 认知记忆\n{context}\n\n",
                "找出语义上互相矛盾的记忆对。例如：\n",
                "- 「邮件批量延迟处理」与「收到紧急邮件会立即响应」— 矛盾\n",
                "- 「组织摩擦时克制介入」与「遇到阻断时主动找执行主体」— 矛盾\n\n",
                "对每对矛盾，输出：\n",
                "ACTION flag_contradiction | memory_id_1 | memory_id_2 | explanation\n\n",
                "规则：\n",
                "- 只标记真正的逻辑矛盾，不是「不同场景的不同策略」\n",
                "- 如果可以共存（如「工作中 X，生活中 Y」），不算矛盾\n",
                "- 宁漏勿错\n",
                "- 无矛盾时输出 NONE",
            ),
            "", // insert_after（由 DAG edges 控制）
        ...
        (
            "integrator",
            "认知整合：根据验证结果和模式发现更新认知层级",
            concat!(
                "你是 Sage 的认知整合器。根据今日的验证结果、教练发现和矛盾检测，做出认知更新决策。\n\n",
                "## 验证结果\n{context}\n\n",
                "可用操作：\n",
                "- ACTION promote_memory | memory_id | new_depth  （提升层级，必须逐级）\n",
                "- ACTION demote_memory | memory_id | new_depth | reason  （降级，有矛盾证据时）\n",
                "- ACTION archive_challenged | memory_id | reason  （归档被反复挑战的认知）\n\n",
                "层级定义（严格）：\n",
                "- episodic → semantic：同一模式出现 3+ 次\n",
                "- semantic → procedural：validation_count ≥ 5，稳定的行为策略/思维框架\n",
                "- procedural → axiom：validation_count ≥ 10，confidence ≥ 0.9，即使环境改变也不会放弃的身份/价值观。每次最多 1 条。\n",
                "- 反例：工作习惯、效率策略、沟通技巧 → 永远不是 axiom\n\n",
                "降级规则：\n",
                "- 被 CHALLENGE 3 次且无 CONFIRM → 降级一层\n",
                "- 被标记为矛盾 → 保留更有证据支持的那条，降级另一条\n\n",
                "输出 NONE 如果不需要任何调整",
            ),
            "", // insert_after（由 DAG edges 控制）
            "每行一个 ACTION",
            "promote_memory,demote_memory,archive_challenged",
            "verification_results,coach_insights,contradiction_results,depth_summary",
            5,
            "",
            false,
        ),
