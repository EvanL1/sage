...
impl OnboardingState {
    ...
    pub fn preview_sop(&self) -> String {
        let lang = &self.partial_profile.identity.prompt_language;
        crate::profile::generate_sop_lang(&self.partial_profile, lang)
    }
    ...
    fn handle_basic_info(&mut self, data: Value) -> Result<()> {
        ...
        let info: BasicInfo = serde_json::from_value(data).context(
            "BasicInfo 数据格式无效，期望 {name, role, primary_language?, secondary_language?}",
        )?;
    ...
    fn handle_schedule(&mut self, data: Value) -> Result<()> {
        ...
        let sd: ScheduleData = serde_json::from_value(data).context(
            "Schedule 数据格式无效，期望 {morning_brief_hour, evening_review_hour, ...}",
        )?;
    ...
    fn handle_stakeholders(&mut self, data: Value) -> Result<()> {
        ...
        let sd: StakeholdersData = serde_json::from_value(data).context(
            "Stakeholders 数据格式无效，期望 {stakeholders: [{name, role, relationship}]}",
        )?;
...
mod tests {
    ...
    fn test_full_onboarding_flow() {
        ...
        state
            .submit_step(json!({
                "reporting_line": ["Alex", "Jordan (Director)", "Sam (CTO)"]
            }))
            .unwrap();
        ...
        state
            .submit_step(json!({
                "projects": [
                    {"name": "ProjectY", "description": "工厂能源监控平台", "status": "Active"},
                    {"name": "ProjectX", "description": "Rust 架构 EMS", "status": "Active"}
                ]
            }))
            .unwrap();
        ...
        state
            .submit_step(json!({
                "stakeholders": [
                    {"name": "Jordan", "role": "Director", "relationship": "直属上级"},
                    {"name": "Sam", "role": "CTO", "relationship": "上级的上级"}
                ]
            }))
            .unwrap();
        ...
        assert_eq!(profile.identity.name, "Alex");
    ...
    fn test_submit_after_completed_returns_error() {
        ...
        // 快速走完所有步骤
        state
            .submit_step(json!({"name": "A", "role": "B"}))
            .unwrap();
