                ...
                repeat with f in allFolders
                    try
                        ...
                        if folderName is "收件箱" or folderName is "Inbox" then
                            set cutoff to (current date) - 604800
                            set unreadMsgs to (messages of f whose is read is false and time sent > cutoff)
                        else if folderName is "已发送邮件" or folderName is "已发送" or folderName is "Sent Items" or folderName is "Sent" then
                            -- 已发送：取最近 5 封（不管已读）
                            set isSent to true
                            set allSent to messages of f
                            set sentCount to count of allSent
                            if sentCount > 5 then set sentCount to 5
                            set unreadMsgs to items 1 thru sentCount of allSent
                            ...
                            if msgCount > 0 then
                                ...
                                repeat with i from 1 to maxCount
                                    ...
                                    set msgDir to "received"
                                    if isSent then set msgDir to "sent"
                                    set output to output & "SUBJECT:" & subject of msg & "||FROM:" & senderAddr & "||DATE:" & (time sent of msg as string) & "||PRIORITY:" & msgPriority & "||DIRECTION:" & msgDir & "||BODY:" & bodyPreview & "|||"
                                end repeat
                            end if
                        end if
                    end try
...
fn format_email_digest(raw: &str) -> String {
    ...
    for entry in &entries {
        ...
        let preview_suffix = if clean.chars().count() > 150 {
            "..."
        } else {
            ""
        };
...
fn parse_emails(raw: &str) -> Vec<Event> {
    raw.split("|||")
        ...
        .filter_map(|entry| {
            ...
            let mut direction = String::from("received");
...
mod tests {
    ...
    fn test_parse_emails_with_body() {
        ...
        let raw = "SUBJECT:Meeting||FROM:sam@test.com||DATE:2026-03-10||BODY:Hi team, let's meet at 3pm to discuss the project.|||";
        let result = parse_emails(raw);
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].title, "Meeting");
        assert!(result[0].body.contains("Hi team, let's meet at 3pm"));
        assert!(result[0].body.starts_with("From: sam@test.com"));
    }
    ...
    fn test_parse_emails_with_priority() {
        let raw = "SUBJECT:Urgent Fix||FROM:boss@example.com||DATE:2026-03-10||PRIORITY:high||BODY:Need fix ASAP|||";
        let result = parse_emails(raw);
        assert_eq!(result.len(), 1);
