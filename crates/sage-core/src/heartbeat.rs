...
pub fn evaluate(config: &Config, schedule: Option<&WorkSchedule>) -> Vec<Action> {
    ...
    // Mirror 周报：周报日 + 1 小时后触发（留出时间收集最后信号）
    if weekday == report_day && hour >= report_hour + 1 {
        actions.push(Action::MirrorWeekly);
    }
...
mod tests {
    ...
    fn make_config(email_enabled: bool, calendar_enabled: bool) -> Config {
        use crate::config::{
            AgentConfig, ChannelsConfig, DaemonConfig, FeedConfig, HooksConfig, MemoryConfig,
