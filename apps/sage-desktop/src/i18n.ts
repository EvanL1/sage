...
const translations = {
  ...
  // FeedIntelligence — additional strings
  "feed.topicPlaceholder": { zh: "搜索主题，如 energy storage、AI agent、Rust async...", en: "Search topics, e.g. energy storage, AI agent, Rust async..." },
  "feed.nlPlaceholder": { zh: "告诉 Sage 要关注什么，例如「添加储能和 AIDC 话题」", en: "Tell Sage what to follow, e.g. \"Add energy storage and AIDC topics\"" },
  "feed.modeSearch": { zh: "搜索", en: "Search" },
  "feed.modeConfig": { zh: "配置", en: "Config" },
  "feed.switchToConfig": { zh: "切换到订阅配置", en: "Switch to subscription config" },
  "feed.switchToSearch": { zh: "切换到主题搜索", en: "Switch to topic search" },
  "feed.topicSearching": { zh: "搜索 + AI 分析中...", en: "Searching + AI analysis..." },
  ...
  "feed.clickExpand": { zh: "点击展开完整简报", en: "Click to expand full briefing" },
  "feed.statsUnread": { zh: "未读条目", en: "Unread Items" },
  "feed.statsHighScore": { zh: "高分推荐", en: "High Score" },
  "feed.statsLearned": { zh: "已学习", en: "Learned" },
  "feed.statsAvgScore": { zh: "平均分", en: "Avg Score" },
  "feed.learnedLabel": { zh: "🧠 学习结果", en: "🧠 Learning Results" },
  "feed.readingNote": { zh: "📖 阅读笔记", en: "📖 Reading Note" },
  "feed.actionLearned": { zh: "🧠 已学习", en: "🧠 Learned" },
  "feed.actionArchived": { zh: "📦 已归档", en: "📦 Archived" },
  "feed.deepLearnBusy": { zh: "学习中...", en: "Learning..." },
  "feed.deepLearn": { zh: "🔬 深入学习", en: "🔬 Deep Learn" },
  "feed.deepLearnFailed": { zh: "学习失败", en: "Deep learn failed" },
  "feed.markKnown": { zh: "✓ 已知", en: "✓ Known" },
  "feed.unarchive": { zh: "↩ 恢复", en: "↩ Restore" },
  "feed.summarizeFromMemory": { zh: "从记忆总结", en: "Summarize from memory" },
  ...
  "settings.memoryEvolutionHint": { zh: "去重、合成特征、精简冗长、关联图谱、衰减旧记忆、晋升验证", en: "Deduplicate, synthesize, condense, link graph, decay stale, promote" },
  ...
  // ── Pipeline page ─────────────────────────────────────────────────────────
  "pipeline.title": { zh: "认知管线", en: "Cognitive Pipeline" },
  "pipeline.showConfig": { zh: "查看原始配置", en: "Show Raw Config" },
  "pipeline.hideConfig": { zh: "隐藏配置", en: "Hide Config" },
  "pipeline.eveningReview": { zh: "晚间认知链", en: "Evening Review" },
  "pipeline.weeklyReport": { zh: "周报链", en: "Weekly Report" },
  "pipeline.stages": { zh: "个阶段", en: "stages" },
  "pipeline.noRuns": { zh: "尚未运行", en: "no runs yet" },
  "pipeline.core": { zh: "核心", en: "CORE" },
  "pipeline.disabled": { zh: "已禁用", en: "DISABLED" },
  "pipeline.ok": { zh: "成功", en: "ok" },
  "pipeline.skip": { zh: "跳过", en: "skip" },
  "pipeline.err": { zh: "错误", en: "err" },

  "pipeline.observer": { zh: "观察者", en: "Observer" },
  "pipeline.observer.desc": { zh: "原始事件 → 语义标注（频率 + 意图分析）", en: "Raw events → semantic annotation with frequency + intent" },
  "pipeline.coach": { zh: "教练", en: "Coach" },
  "pipeline.coach.desc": { zh: "标注后的观察 → 行为模式（情景/语义/程序性记忆）", en: "Annotated observations → behavioral patterns" },
  "pipeline.mirror": { zh: "镜像", en: "Mirror" },
  "pipeline.mirror.desc": { zh: "行为模式 → 反思洞见，推送给用户", en: "Patterns → reflections surfaced to user" },
  "pipeline.questioner": { zh: "提问者", en: "Questioner" },
  "pipeline.questioner.desc": { zh: "生成苏格拉底式问题，最大化信息增益", en: "Generates Socratic questions with max-info-gain priority" },
  "pipeline.evolution": { zh: "记忆演化", en: "Evolution" },
  "pipeline.evolution.desc": { zh: "变换（合并+提炼+精简）→ 图谱（关联+衰减+晋升）", en: "transform (merge+synth+condense) → graph (link+decay+promote)" },
  "pipeline.person_observer": { zh: "人物观察者", en: "Person Observer" },
  "pipeline.person_observer.desc": { zh: "从邮件、消息、观察中提取人物认知", en: "Extracts people insights from emails, messages, observations" },
  "pipeline.calibrator": { zh: "校准器", en: "Calibrator" },
  "pipeline.calibrator.desc": { zh: "检测行为模式偏移，生成校准规则", en: "Detects behavioral pattern shifts, generates calibration rules" },
  "pipeline.meta": { zh: "元认知", en: "Meta" },
  "pipeline.meta.desc": { zh: "自我进化：优化管线参数、重写提示词、生成 UI", en: "Self-evolution: optimizes pipeline params, rewrites prompts, generates UI" },
  "pipeline.strategist": { zh: "战略家", en: "Strategist" },
  "pipeline.strategist.desc": { zh: "生成周度战略规划和优先级", en: "Generates weekly strategic plans and priorities" },
  "pipeline.mirror_weekly": { zh: "周度镜像", en: "Weekly Mirror" },
  "pipeline.mirror_weekly.desc": { zh: "周度反思与成长轨迹分析", en: "Weekly reflection and growth trajectory analysis" },

  "pipeline.custom": { zh: "自定义", en: "CUSTOM" },
  "pipeline.addStage": { zh: "添加认知阶段", en: "Add Cognitive Stage" },
  "pipeline.addAfter": { zh: "在此之后插入阶段", en: "Insert stage after this" },
  "pipeline.after": { zh: "之后", en: "after" },
  "pipeline.stageName": { zh: "阶段名称", en: "Stage name" },
  "pipeline.stageDesc": { zh: "描述", en: "Description" },
  "pipeline.stageDescPlaceholder": { zh: "这个阶段做什么", en: "What this stage does" },
  "pipeline.stagePrompt": { zh: "Prompt 模板", en: "Prompt template" },
  "pipeline.contextHint": { zh: "用 {context} 引用今日观察数据", en: "Use {context} to reference today's observations" },
  "pipeline.promptPlaceholder": { zh: "基于今日观察：\n{context}\n\n请分析...", en: "Based on today's observations:\n{context}\n\nPlease analyze..." },
  "pipeline.create": { zh: "创建", en: "Create" },
  "pipeline.outputFormat": { zh: "输出格式", en: "Output Format" },
  "pipeline.outputFormatHint": { zh: "约束 LLM 的输出结构，留空则自由输出", en: "Constrains LLM output structure. Leave empty for free-form" },
  "pipeline.outputFormatPlaceholder": { zh: "例如：每行一条，格式 `[类别] 内容`\n或 JSON: {\"risks\": [...], \"actions\": [...]}", en: "e.g. One item per line: `[CATEGORY] content`\nOr JSON: {\"risks\": [...], \"actions\": [...]}" },
  "pipeline.morningBrief": { zh: "晨间简报链", en: "Morning Brief" },
  "pipeline.dataFlows": { zh: "数据采集", en: "Data Collection" },
  "pipeline.periodicFlows": { zh: "周期性智能", en: "Periodic Intelligence" },
  "pipeline.heartbeat": { zh: "心跳", en: "heartbeat" },
  "pipeline.actions": { zh: "可用动作", en: "Available Actions" },
  "pipeline.actionsHint": { zh: "赋予此阶段执行能力——LLM 可输出 ACTION 命令自动执行", en: "Give this stage execution ability — LLM can output ACTION commands" },

  // ── People page ───────────────────────────────────────────────────────────
  "people.title": { zh: "人物", en: "People" },
  "people.extractNow": { zh: "立即提取", en: "Extract Now" },
  "people.extracting": { zh: "提取中...", en: "Extracting..." },
  "people.noData": { zh: "尚未观察到任何人物", en: "No people observed yet" },
  "people.noDataHint": { zh: "Sage 会在晚间回顾时从你的邮件、聊天和对话中学习人物信息", en: "Sage learns about people from your emails, chats, and conversations during evening review" },
