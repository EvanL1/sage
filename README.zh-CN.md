...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/测试-478%20通过-brightgreen?style=flat-square" />
...
│  │              └→ person_observer           │   │
│  │ calibrator   strategist（可并行）          │   │
│  │                                          │   │
│  │ evolution: merge→synth→condense→link     │   │
│  │            →decay→promote                │   │
│  │                                          │   │
│  │ meta: params→prompts→ui（自进化）         │   │
│  └──────────────────────────────────────────┘   │
│                                                 │
│  ┌───────────────┐  ┌────────────────────────┐  │
│  │ 任务引擎      │  │ 反思信号检测器          │  │
...
### 认知管线（自进化 DAG）
16 个预设 stage 以 DAG（有向无环图）模式执行，而非固定线性链。所有 stage 运行在**统一约束引擎**中：
- **ACTION 白名单**：每个 stage 精确声明可执行的写操作（22 种 ACTION 类型）
- **输入源过滤**：stage 只能读取其被授权的数据源
- **速率限制**：每次执行的最大 ACTION 数量
- **前置条件 SQL 门控**：可选的 SQL 查询，必须通过才执行
- **Meta stage**（`meta_params`、`meta_prompts`、`meta_ui`）：管线自我进化——根据执行历史调整参数、重写 prompt、生成 UI 页面
...
### 双语界面
完整中英文界面，530 个翻译键。语言跟随 profile 设置即时切换——无需重启。所有 LLM prompt 同样遵循语言偏好。
...
| 存储 | **SQLite** — WAL 模式，FTS5 全文搜索，49 次迁移 |
| LLM | 多 Provider 优先级队列 + 按模型独立配置 |
| 国际化 | 零依赖双语系统（en/zh），530 个翻译键 |
...
```
sage/
├── apps/sage-desktop/             # Tauri 2 桌面应用
│   ├── src/                       # React 18 前端（15 个页面）
│   │   ├── i18n.ts                # 双语翻译（530 个键）
│   │   ├── LangContext.tsx        # 语言上下文 Provider
│   │   └── pages/                 # Dashboard, Chat, Tasks, Settings, ...
│   └── src-tauri/                 # Rust 后端（commands, tray, daemon）
├── crates/
│   ├── sage-types/                # 共享类型（Event, Memory, Message 等）
│   ├── sage-store/                # SQLite 持久化（217 个测试，49 次迁移）
│   ├── sage-llm/                  # LLM Provider 抽象层（6 个测试）
│   ├── sage-channels/             # 输入通道：邮件、日历、RSS（10 个测试）
│   ├── sage-core/                 # 业务逻辑（203 个测试）
│   │   ├── daemon.rs              # 后台事件循环
│   │   ├── pipeline.rs            # DAG 认知管线引擎
│   │   ├── pipeline/              # invoker, actions, stages, parser
│   │   ├── router.rs              # 事件路由 + Skill 分发
│   │   ├── task_intelligence.rs   # 任务信号检测
│   │   ├── reflective_detector.rs # 规则引擎反思信号检测
│   │   └── ...
│   └── sage-cli/                  # CLI 二进制（薄封装）
...
cargo test --workspace        # 运行全部测试（478）
...
```
后台 Daemon（事件循环）：
  tick() → 邮件/日历轮询 → 时间窗口检查 → 认知管线
  → DAG 调度 16 个 stage → ACTION 约束系统 → SQLite

认知管线（Evening，16 个 stage）：
  observer → coach → mirror → questioner → calibrator
  → person_observer, strategist（可并行）
